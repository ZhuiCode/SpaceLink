...
func DialAddr(addr string, tlsConf *tls.Config, config *Config) (Session, error) {
	...
	// Create the conn.PconnManager here. It will be used to manage UDP connections
	pconnMgr := &conn.PconnManager{perspective: protocol.PerspectiveClient}
...
func DialNonFWSecure(
	...
	var pconnMgr *conn.PconnManager
...
func (c *client) establishSecureConnection(conn conn.Connection) error {
